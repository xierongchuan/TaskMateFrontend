<x-layouts.app>
    <div class="mb-6" x-data="taskCreate">
        <!-- Breadcrumbs -->
        <div class="mb-6 flex items-center text-sm">
            ...
            <span class="text-gray-500 dark:text-gray-400">{{ __('Create Task') }}</span>
        </div>

        <!-- Page Title -->
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ __('Create New Task') }}</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-1">{{ __('Fill in the details to create a new task') }}</p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 mt-6">
            ...
            <div x-show="loadingData" class="text-center py-12">
                ...
                <p class="mt-4 text-gray-600 dark:text-gray-400">{{ __('Loading form data...') }}</p>
            </div>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('taskCreate', () => ({
                ...
                async loadFormData() {
                    ...
                    try {
                        // Load dealerships and users in parallel
                        const [dealershipsData, usersData] = await Promise.all([
                ...
                async submitForm() {
                    ...
                    try {
                        ...
                        // Create task
                        await window.apiClient.createTask(submitData);

                        this.successMessage = '{{ __('Task created successfully!') }}';
                        ...
                        this.errorMessage = error.message || '{{ __('Failed to create task. Please try again.') }}';
