<x-layouts.app>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700" x-data="userEdit">
        <!-- Loading State -->
        ...
        <div x-show="loading === false && error" class="text-center py-12 text-red-600 dark:text-red-400 p-6">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 mx-auto mb-3" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <form x-show="loading === false && !error" @submit.prevent="submitForm" class="p-6 space-y-6">
            ...
            <div>
                ...
                <p x-show="errors && errors.full_name" class="mt-1 text-sm text-red-600 dark:text-red-400"
            ...
            <div>
                ...
                <input type="tel" id="phone" x-model="formData.phone"
                    ...
                    @input="formData.phone_number = $el.value">
                <p x-show="errors && errors.phone" class="mt-1 text-sm text-red-600 dark:text-red-400"
            ...
            <div>
                ...
                <p x-show="errors && errors.role" class="mt-1 text-sm text-red-600 dark:text-red-400"
                    x-text="errors.role"></p>
            </div>
            ...
            <div>
                ...
                <p x-show="errors && errors.dealership_id" class="mt-1 text-sm text-red-600 dark:text-red-400"
                    x-text="errors.dealership_id"></p>
            </div>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('userEdit', () => ({
                ...
                async init() {
                    ...
                    // Load dealerships first, then user data
                ...
                async waitForApiClient() {
                    ...
                    while ((!window.apiClientReady || !window.apiClient || !window.apiClient.getDealerships) && attempts < maxAttempts) {
                ...
                async loadDealerships() {
                    try {
                        ...
                        const data = await window.apiClient.getDealerships({
                            per_page: 100,
                            is_active: true
                        });
                        console.log('Dealerships loaded:', data);
                        this.dealerships = data.data || [];
                    } catch (error) {
                        console.error('Error loading dealerships:', error);
                        this.dealerships = []; // Ensure dealerships is array
                    }
                },
                async loadUser() {
                    this.loading = true;
                    ...
                    try {
                        ...
                        console.log('API User Data:', data); // Debug API response

                        // Handle different response structures
                        const userData = data.data || data; // Some APIs return {data: {...}} others return the object directly

                        // Update form data with proper field mapping
                        this.formData.login = userData.login || '';
                        this.formData.full_name = userData.full_name || '';
                        this.formData.telegram_id = userData.telegram_id || '';

                        // Handle phone field - ensure both phone and phone_number are synchronized
                        ...
                        this.formData.role = userData.role || '';
                        this.formData.dealership_id = userData.dealership_id || '';

                        console.log('Form Data after update:', this.formData); // Debug form data

                        // Force Alpine to re-render the form
                        await this.$nextTick();
                ...
                async submitForm() {
                    ...
                    try {
                        ...
                        if (!window.apiClientReady || !window.apiClient || !window.apiClient
                            .updateUser) {
                        ...
                        const submitData = {
                            ...
                            phone_number: this.formData.phone_number || this.formData.phone, // Use phone_number if available
                        ...
                        this.errorMessage = error.message ||
                            '{{ __('Failed to update employee. Please try again.') }}';
